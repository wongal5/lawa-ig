          ...
          <span className='profile-sub-data'>
            <Modal size="mini" trigger={<span><span className='profile-sub-data-num'>{props.user.followers.length} </span>followers</span>}>
              ...
              <Modal.Content scrolling><List divided verticalAlign='middle'>{props.user.followers.map((follower) => {
                return <List.Item><span className="space"><List.Content className="follow">{follower}</List.Content></span><List.Content className="small-button" floated="right"><Button color="blue" size="tiny">Following</Button></List.Content></List.Item>;
              })}</List></Modal.Content>
            </Modal></span>
          <span className='profile-sub-data'>
            <Modal size="mini" trigger={<span><span className='profile-sub-data-num'>{props.user.following.length} </span>followers</span>}>
              ...
              <Modal.Content scrolling><List divided verticalAlign='middle'>{props.user.following.map((following) => {
                return <List.Item><List.Content className="follow">{following}</List.Content><List.Content className="small-button" floated="right"><Button color="blue" size="tiny">Following</Button></List.Content></List.Item>;
              })}</List></Modal.Content>
            </Modal></span>
        </Grid.Row>
        <Grid.Row>
