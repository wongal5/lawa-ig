          ...
          <span className='profile-sub-data'>
            <Modal size="mini" trigger={<span><span className='profile-sub-data-num'>{props.user.following.length} </span>followers</span>}>
              ...
              <Modal.Content scrolling><List divided verticalAlign='middle'>{props.user.following.map((following) => {
                return <List.Item key={following.user_id}><Image avatar src={following.prof_pic} /><List.Content className="follow">{following.name}</List.Content><List.Content className="small-button" floated="right"><Button color="blue" size="mini">Following</Button></List.Content></List.Item>;
              })}</List></Modal.Content>
            </Modal></span>
        </Grid.Row>
        <Grid.Row>
...
>>>>>>> added key property to modal mapping in profile panel followers and following
